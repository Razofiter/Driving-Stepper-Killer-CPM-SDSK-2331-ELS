...
%%
% Trebuie sa fac o verificare de RMS pentru a cunoaste cat timp pot sa
% execut miscarea de excitatie intr-o configuratie data.
%%
...
Start_pwmPeriod = SemiRas_pwmPeriod;
for r = 1: length(rot)
...
for r = 1: length(rot)
    for c = 1:length(NrImpulsuriRotatie)
        if length(new_pwmPeriod_mat{(r-1)*length(NrImpulsuriRotatie)+c}) > steps_in_relation_to_distance(r,c)
            ...
            sprintf('In configuratia %d,%d se va putea realiza deplasarea maxima de %d din valoarea dorita!\r',r,c, length(new_pwmPeriod_mat{(r-1)*length(NrImpulsuriRotatie)+c})/steps_in_relation_to_distance(r,c))
        end
    end
end

% Verificam valoarea frecventei care va fi obitnuta
quarter_period = sum(new_pwmPeriod_mat{24})*10^(-6)+ras/2+Start_pwmPeriod(3,6)*10^(-6);
actual_frequency = 1/(4*quarter_period)
%quarter_period = sum(new_pwmPeriod_mat{24})*10^(-6)+ras/2
...
% Scriem valorile obtinute intr-un fisier de tip text pentru a le folosi in
% dezvoltarea programului scris in C, din keilUvision
csvwrite('valoriPWM.txt',new_pwmPeriod_mat{24}');
csvwrite('valoriPWM_verification.txt',new_pwmPeriod_mat{24});
